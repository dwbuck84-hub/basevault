...
try {
    ...
    const singleLineAnchor = "const connectWallet = async () => {";
  ...
  const connectWallet = async () => {`;
    ...
    if (code.includes(singleLineAnchor) && !code.includes('executeWeb3Listing')) {
        // We replace the single line with our engines, and then put the single line back at the bottom
        code = code.replace(singleLineAnchor, allEngines);
        fs.writeFileSync('app/page.tsx', code, 'utf8');
        console.log("✅ ENGINES INJECTED: Listing, Buy Now, and Bid functions are locked and loaded.");
    } else if (code.includes('executeWeb3Listing')) {
        console.log("⚠️ ALREADY INJECTED: The Web3 engines are already in the file.");
    } else {
        console.log("❌ SCRIPT FAULT: Could not find 'const connectWallet = async () => {'.");
    }
