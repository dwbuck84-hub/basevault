...
try {
  ...
  const formatTimeLeft = (endTime) => {
    ...
    // 2. Swap the static 'AUCTION' badge for the Live Countdown
    ...
    const categoryInjection = `<span className="truncate pr-2">{item.type === 'digital' ? \`💼 \${item.category}\` : \`📦 \${item.category}\`}</span>
                          <span className="text-amber-400 flex-shrink-0 text-[10px] font-black">⭐ {item.seller_rating ? Number(item.seller_rating).toFixed(1) : 'NEW'}</span>`;
    ...
    console.log("✅ UI UPGRADED: Safe injection complete. No duplicates.");
} catch(e) { 
