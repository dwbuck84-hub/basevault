...
try {
    ...
    // 4. Inject the TypeScript-Safe sorting logic
    const mapTarget = "visiblySlicedListings.map(item =>";
    const mappedInjection = `[...visiblySlicedListings].sort((a, b) => {
                  const priceA = Number(a.highestBid) > 0 ? Number(a.highestBid) : Number(a.reservePrice);
                  const priceB = Number(b.highestBid) > 0 ? Number(b.highestBid) : Number(b.reservePrice);
    ...
    if (!code.includes("const priceA = Number")) {
        code = code.split(mapTarget).join(mappedInjection);
    }

    fs.writeFileSync('app/page.tsx', code, 'utf8');
    console.log("✅ UI UPGRADED: Strict TypeScript compliance achieved.");
} catch(e) { 
