...
try {
    ...
    // 1. Wipe the improperly scoped block from the top
    const marker = "// 🔥 WEB3 ENGINES: LISTING, BUY, BID 🔥";
    if (code.includes(marker)) {
        const start = code.indexOf(marker);
        const endMarker = "alert(\"❌ Error: \" + (error.reason || error.message));\n    }\n  };";
        const end = code.indexOf(endMarker, start) + endMarker.length;
        code = code.substring(0, start) + code.substring(end);
    }

    // 2. The exact engines code
    const allEngines = `
  ...
  const executeBid = async (listingId: number, bidAmountStr: string, isUSDC: boolean) => {
    ...
    // 3. Inject correctly right above connectWallet
    const anchor = "const connectWallet = async () => {";
    if (code.includes(anchor)) {
        code = code.replace(anchor, allEngines + "  " + anchor);
        fs.writeFileSync('app/page.tsx', code, 'utf8');
        console.log("✅ SCOPE FIXED: Engines moved below state declarations.");
    } else {
        console.log("❌ SCRIPT FAULT: Could not find connectWallet anchor.");
    }
